...
仅启用且未封禁的管理员可通过邮箱密码、Passkey 或 GitHub OAuth 获取会话；旧的普通用户会话不能使用认证管理接口。邮箱密码由 Better Auth 的密码哈希实现处理。新设置或管理员重置的密码至少需要 12 个字符。

同一账号或 IP 在 15 分钟内连续失败 5 次后，邮箱密码登录会暂时受限。限流状态记录在数据库中，支持多实例部署。

### Passkey

仅管理员可以登录并在设置页面注册 Passkey。订阅用户只使用管理员交付的订阅链接，无需激活、设置密码或登录。历史激活链接及其设置接口已经停用。
