...
- 订阅交付：普通用户创建后直接复制订阅或 Shadowrocket 链接，不生成激活链接、不设置密码。旧激活入口只展示停用提示；只有启用且未封禁的管理员可以获取登录会话。
- 设置：身份、认证、登录会话和外观。不得添加不可用的通知开关、伪备份按钮、没有实际效果的主题选项。

## 视觉语言

使用 `src/styles/workspace.css` 和 `src/styles/product.css`。主操作为青柠色，信息强调为电光蓝，纸色/深绿色背景保持同一套结构。品牌资产仅复用 `BrandMark` 与站点图标；不从第三方动态加载素材或字体。

信息结构靠排版、留白、细分隔线和关系组织，不靠每个字段一个卡片。抽屉、表单与确认对话框可以有明确容器，但不要为了装饰重复嵌套。

使用 Lucide 的一致描边、Radix 的对话框与焦点管理；轻量 CSS 仅用于必要反馈。新增组件应能解释为何比已有实现更好，不能只为组件数量添加依赖。
...
不添加宣传式大标题、英文眉题或整屏介绍。页面 h1 桌面不超过 24px、手机不超过 22px；首屏直接显示列表、数据和操作。没有实际操作价值的说明移入文档，不在页面反复解释。

手机端不运行装饰循环动画，不使用全屏模糊叠层。命令搜索、账户编辑器、YAML 编辑器按需加载；列表链接不预取全部目标页面。Shadowrocket 复制按钮直接放在账户行操作区，触摸目标至少 44px。
...
Shadowrocket 与普通订阅复制复用 `SubscriptionActions` 和 `Action`；列表使用 quiet 操作组，详情和创建完成态使用相同的 secondary 按钮。不得单独添加品牌色底、不同字号/圆角或特殊悬停样式。两种链接均保留 Lucide 图标、可读名称与直接入口；窄屏自然换行，点击区不小于 44px。复制回调必须在原点击事件内同步发起，不能为动画或加载效果延迟剪贴板调用。
