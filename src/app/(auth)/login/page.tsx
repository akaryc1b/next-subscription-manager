...
import { Github, Key, Loader2, Layers3, ShieldCheck, Sparkles } from 'lucide-react';
      ...
      <div className="relative z-10 grid w-full max-w-6xl gap-6 lg:grid-cols-[1fr_28rem] lg:items-center">
        <section className="hidden lg:block">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-border bg-background-secondary px-4 py-2 text-sm text-foreground-secondary backdrop-blur-xl">
            ...
            iOS 26 Liquid Glass workspace
          </div>
          <h1 className="max-w-2xl text-5xl font-semibold tracking-tight text-foreground-primary">
            Manage subscriptions in a calm macOS-style control center.
          </h1>
          <p className="mt-5 max-w-xl text-lg text-foreground-secondary">
            Users, YAML configs, access monitoring, and security settings are organized as soft glass panels with focused admin workflows.
          </p>
          <div className="mt-8 grid max-w-2xl gap-3 sm:grid-cols-3">
            {[
              { icon: ShieldCheck, label: 'Secure auth' },
              { icon: Layers3, label: 'Glass panels' },
              { icon: Sparkles, label: 'macOS layout' },
            ].map((item) => {
              const Icon = item.icon;
              return (
                <div key={item.label} className="rounded-3xl border border-border bg-background-secondary p-4 backdrop-blur-xl">
                  <Icon className="mb-3 h-5 w-5 text-accent-primary" />
        ...
        <Card className="mx-auto w-full max-w-md">
          <CardContent className="p-6 sm:p-7">
            <div className="mb-7 flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-accent-primary text-white shadow-lg">
