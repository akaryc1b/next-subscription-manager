...
export const dateStringSchema = z.string().trim().refine(value => !Number.isNaN(Date.parse(value)), '日期格式无效')
...
const optionalPasswordSchema = z.preprocess(value => {
  ...
  return value
}, z.string().min(12, '密码至少 12 个字符').max(128, '密码最多 128 个字符').optional())
...
export const userCreateSchema = z.object({
  ...
  if (data.role === 'user' && data.password) ctx.addIssue({ code: 'custom', path: ['password'], message: '订阅用户不能设置登录密码' })
  if (data.role === 'admin' && !data.password) ctx.addIssue({ code: 'custom', path: ['password'], message: '管理员必须设置密码' })
})
...
export function formatZodError(error: z.ZodError) { return error.issues[0]?.message || '请求参数无效' }
