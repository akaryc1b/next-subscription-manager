...
export function securityCopy(type: string, reason?: unknown): { title: string; detail: string; nextStep: string } {
  ...
  const labels: Record<string, [string, string, string]> = {
    ...
    activation_setup_success: ['账户激活成功', '账户已完成激活', '历史激活记录；订阅用户不再提供登录入口。'],
    activation_token_expired: ['激活链接已到期', '本次激活没有完成', '激活入口已停用，请交付订阅链接。'],
    activation_token_used: ['激活链接已使用', '本次激活没有重复执行', '激活入口已停用，订阅用户无需登录。'],
