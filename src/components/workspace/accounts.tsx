...
export async function copyAccountLink(id: string, shadowrocket = false) {
  // Fetch metadata, never the quota-consuming /api/sub/:token endpoint.
...
export function ProfilePicker({ selected, onChange }: { selected: string[]; onChange: (ids: string[]) => void }) {
  ...
  return (
    <div className="o-config-picker">
      ...
      {resource.data?.configs.map(config => (
        <label className="o-picker-row" key={config.id}>
          <input type="checkbox" checked={selected.includes(config.id)} onChange={() => onChange(selected.includes(config.id) ? selected.filter(id => id !== config.id) : [...selected, config.id])}/>
          <span>{config.name}</span><small>{config.isActive ? '已启用' : '已停用'}</small>
        </label>
      ))}
      ...
      <div className="o-pager" style={{ padding: '10px 12px' }}>
        <span>已选择 {selected.length} 份；停用配置不会分发。</span>
        {selected.length > 0 && <Action variant="quiet" onClick={() => onChange([])}>清空选择</Action>}
      </div>
    </div>
  )
}
...
function AccountEditor({ account, onClose, onChanged }: { account: Account | null; onClose: () => void; onChanged: () => void }) {
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      ...
      if (account) {
        ...
        toast.success('账户与授权已保存')
        onChanged()
        onClose()
      } else {
        const result = await request<{ activationLink: string | null }>('/api/users', { method: 'POST', body: JSON.stringify(payload) })
        setActivation(result.activationLink)
        setCreated(true)
        onChanged()
      }
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : '保存未完成')
    } finally {
      setBusy(false)
    }
  }
  const saveQuota = async () => {
    ...
    if (quota.trim() === '' || !Number.isInteger(value) || value < 0 || value > 2147483647) {
      setError('请输入 0 到 2147483647 之间的整数，0 表示不限制。')
      return
    }
    setBusy(true)
    setError('')
    try {
      ...
      toast.success('访问额度已更新')
      onChanged()
      onClose()
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : '额度未更新')
    } finally {
      setBusy(false)
    }
  }
  const copy = async (rocket = false) => {
    if (!account || busy) return
    setBusy(true)
    try { await copyAccountLink(account.id, rocket) }
    catch (reason) { setError(reason instanceof Error ? reason.message : '复制失败，请检查浏览器剪贴板权限。') }
      ...
      <Drawer title={created ? '账户已准备好' : account ? account.email : '创建订阅账户'} description={account ? '查看授权、分配配置，或处理订阅访问条件。' : '先确定谁可以用，再决定可以用什么。'} onClose={close}>
        {created ? (
          <div className="o-drawer-body">
            ...
            {activation ? (
              <>
                <div className="o-insight"><h3>把激活链接交给用户</h3><p>用户通过它设置自己的登录密码。链接包含访问凭据，请私下发送。</p></div>
                <div className="o-activation-link">{activation}</div>
                <Action variant="primary" onClick={() => { void navigator.clipboard.writeText(activation).then(() => toast.success('已复制激活链接')).catch(() => toast.error('复制失败，请手动选中链接复制')) }}><Copy/>复制激活链接</Action>
              </>
          ...
          <>
            <div className="o-tabs" style={{ padding: '14px 26px 0' }} aria-label="账户详情视图">
              <button type="button" aria-pressed={tab === 'access'} onClick={() => changeTab('access')}>账户与授权</button>
              {account?.subscription && <button type="button" aria-pressed={tab === 'subscription'} onClick={() => changeTab('subscription')}>链接与额度</button>}
            </div>
            <form onSubmit={save} className="o-drawer-form">
              <div className="o-drawer-body">
                {error && <Problem message={error}/>}
                {tab === 'access' ? (
                  <>
                    <label className="o-field"><span>账户邮箱</span><input className="o-input" type="email" required autoComplete="email" value={form.email} onChange={event => setForm({ ...form, email: event.target.value })}/></label>
                    <div className="o-form-grid">
                      <label className="o-field"><span>账户角色</span><select className="o-select" value={form.role} onChange={event => setForm({ ...form, role: event.target.value as Account['role'] })}><option value="user">订阅用户</option><option value="admin">管理员</option></select></label>
                      <label className="o-field"><span>有效期至（本地时间）</span><input className="o-input" type="datetime-local" step="1" value={form.expiresAt} onChange={event => setForm({ ...form, expiresAt: event.target.value })}/><small>留空表示长期有效。</small></label>
                    </div>
                    {(form.role === 'admin' || account) && (
                      <label className="o-field" style={{ marginTop: 19 }}><span>{account ? '重设密码（可选）' : '管理员密码'}</span><input className="o-input" type="password" autoComplete="new-password" minLength={12} maxLength={128} required={!account && form.role === 'admin'} value={form.password} onChange={event => setForm({ ...form, password: event.target.value })}/><small>{account ? '留空保留原密码；设置新密码会撤销现有会话。' : '至少 12 个字符。普通用户通过激活链接自行设置。'}</small></label>
                    )}
                    <section className="o-form-section">
                      <h3>允许使用的配置</h3>
                      <ProfilePicker selected={form.configIds} onChange={configIds => setForm({ ...form, configIds })}/>
                      {account && account.userConfigs.length > 0 && <div className="o-profile-links" style={{ maxWidth: '100%', marginTop: 12 }}>{account.userConfigs.map(({ config }) => <Link key={config.id} href={configHref(config.id)}>{config.name}<ArrowUpRight size={11}/></Link>)}</div>}
                    </section>
                    {account && (
                      <section className="o-form-section">
                        <h3>账户控制</h3>
                        <p className="o-footnote">更改状态会撤销登录会话。{dirty ? '请先保存或放弃上方修改，再操作状态。' : '订阅权限始终由服务端校验。'}</p>
                        <div className="o-actions" style={{ marginTop: 12 }}>
                          <Action disabled={dirty || busy} onClick={() => setConfirm('pause')}>{account.isActive ? '停用账户' : '恢复账户'}</Action>
                          <Action disabled={dirty || busy} onClick={() => setConfirm('ban')}>{account.isBanned ? '解除封禁' : '封禁账户'}</Action>
                          <Action variant="quiet" disabled={dirty || busy} onClick={() => setConfirm('delete')}>删除账户</Action>
                        </div>
                      </section>
                    )}
                  </>
                  ...
                  <>
                    ...
                    <section className="o-form-section">
                      <h3>访问额度</h3>
                      <dl className="o-facts"><div><dt>已使用</dt><dd>{account.subscription.accessCount.toLocaleString()} 次</dd></div><div><dt>链接更新于</dt><dd>{formatDate(account.subscription.tokenRotatedAt, true)}</dd></div></dl>
                      <label className="o-field" style={{ marginTop: 18 }}><span>允许的总访问次数</span><input className="o-input" type="number" min="0" max="2147483647" step="1" value={quota} onChange={event => setQuota(event.target.value)}/><small>0 表示不限制。这里调整总上限，不会清空已使用次数。</small></label>
                      <Action disabled={busy || formDirty} variant="primary" onClick={() => void saveQuota()}>保存额度</Action>
                    </section>
              ...
              <footer className="o-drawer-footer">
                <Action onClick={close} disabled={busy}>关闭</Action>
                {tab === 'access' ? <Action type="submit" variant="primary" disabled={busy}>{busy ? '正在保存…' : account ? '保存修改' : '创建账户'}</Action> : account && <Link href={`/monitor?userId=${account.id}`} className="o-button">查看访问记录<ArrowUpRight/></Link>}
              </footer>
            </form>
          </>
        )}
...
function AccountLoader({ id, onClose, onChanged }: { id: string; onClose: () => void; onChanged: () => void }) {
  ...
  if (!resource.data?.users[0]) {
    return <Drawer title="账户详情" description="读取账户的授权与订阅条件。" onClose={onClose}><div className="o-drawer-body">{resource.error ? <Problem message={resource.error} retry={resource.reload}/> : resource.loading ? <Loading/> : <Empty title="账户不存在" description="它可能已被删除，请关闭详情并刷新列表。"/>}</div></Drawer>
  }
  ...
  return (
    <div className="o-page">
      ...
      <div className="o-toolbar">
        <div className="o-tabs" aria-label="账户筛选">{accountFilters.slice(0, 3).map(([key, label]) => <button key={key} type="button" aria-pressed={filter === key} onClick={() => { setPage(1); navigate('filter', key) }}>{label}</button>)}</div>
        <label className="o-search"><Search/><input aria-label="搜索账户邮箱" placeholder="搜索邮箱…" value={query} onChange={event => { setQuery(event.target.value); setPage(1) }}/></label>
        <select aria-label="更多账户筛选" className="o-filter-select" value={filter} onChange={event => { setPage(1); navigate('filter', event.target.value) }}>{accountFilters.map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select>
      </div>
      {resource.error && <Problem message={resource.error} retry={resource.reload}/>}
        ...
        <>
          {resource.data.users.length ? (
            <div className="o-table-wrap"><table className="o-table">
              <thead><tr><th>账户</th><th>访问条件</th><th>配置授权</th><th>访问次数</th><th>操作</th></tr></thead>
              <tbody>{resource.data.users.map(account => {
          ...
          ) : <Empty title={search ? '没有找到这个账户' : filter === 'all' ? '还没有订阅账户' : '这个筛选下没有账户'} description={search ? '换一个邮箱关键词再试试。' : filter === 'all' ? '创建账户、分配配置，然后交付订阅链接。' : '可以切换到全部账户，查看完整列表。'} action={<Action onClick={() => { setPage(1); if (search) setQuery(''); else navigate(filter === 'all' ? 'new' : 'filter', filter === 'all' ? '1' : 'all') }}>{search ? '清空搜索' : filter === 'all' ? '创建账户' : '查看全部'}</Action>}/>}
