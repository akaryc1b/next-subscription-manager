      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="o-command orbit-portal">
          ...
          <div className="o-command-input">
            <Search/>
            <input autoComplete="off" role="combobox" aria-expanded="true" aria-controls="workspace-results" aria-activedescendant={index >= 0 ? `workspace-result-${index}` : undefined} aria-label="搜索页面、账户或配置" placeholder="想找什么？输入账户、配置或页面…" value={query}
...
export function WorkspaceShell({ children }: { children: ReactNode }) {
  ...
  const nav = <><Link href="/dashboard" className="o-brand" onClick={() => setMobileOpen(false)} aria-label="sub 订阅工作台"><span><BrandMark/></span><strong>sub<span>.</span></strong></Link><p className="o-nav-caption">你的订阅工作空间</p><nav aria-label="主导航">{navigation.map(item => <Link key={item.href} href={item.href} aria-current={pathname === item.href ? 'page' : undefined} onClick={() => setMobileOpen(false)}><item.icon/><span>{item.label}</span>{pathname === item.href && <motion.i layoutId="nav-marker" transition={{ type: 'spring', stiffness: 380, damping: 32 }} className="o-nav-marker"/>}</Link>)}</nav><div className="o-nav-bottom"><button type="button" className="o-nav-shortcut" onClick={openCommand}><Command/><span>少一点寻找，多一点掌控</span><kbd>⌘ K</kbd></button><div className="o-nav-account"><Avatar name={session?.user.email || 'ME'}/><div><strong>{session?.user.name || '管理员'}</strong><span>{session?.user.email || '当前工作空间'}</span></div><Action variant="quiet" aria-label="退出登录" disabled={signingOut} onClick={() => void signOut()}><LogOut/></Action></div></div></>
