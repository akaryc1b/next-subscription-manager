...
## 管理员登录边界（2026-09-07 修订）

普通订阅用户不登录，不创建激活令牌或密码凭据；创建结果只提供订阅和 Shadowrocket 复制操作。旧 `/activate` 及 verify/setup 接口已经停用，接口统一返回 410，不区分令牌是否存在。激活表仅为数据库向后兼容保留。登录 session 的创建使用当前数据库中的 role/isActive/isBanned 判定，既有普通用户会话不能继续使用认证管理端点。将订阅用户升级为管理员需要新密码并清除历史认证绑定，避免继承曾由订阅用户控制的凭据。详见 `admin-only-accounts.md`。
